...
public class SceneManager {
    ...
    public void showWinScene() {
        ...
        AnimationTimer fireworksTimer = new AnimationTimer() {
            ...
            private int particleCount = 0; // Debug counter

            @Override
            public void handle(long now) {
                if (now - lastUpdate >= 16_666_666) { // ~60 FPS
                    gc.clearRect(0, 0, canvas.getWidth(), canvas.getHeight());
                    gc.setFill(Color.BLACK); // Ensure dark background
                    gc.fillRect(0, 0, canvas.getWidth(), canvas.getHeight());

                    if (now - lastFirework >= 300_000_000) { // Every 0.3 seconds
                        double x = random.nextDouble() * 600 + 100; // Center within 100-700 x-range
                        double y = random.nextDouble() * 300 + 150; // Center within 150-450 y-range
                        for (int i = 0; i < 100; i++) { // Increased to 100 particles
